...
class Tests < Test::Unit::TestCase
  ...
  def test_filtering
    ...
    filtered1 = m1.filter_list(
      list: %w[tummy rummy rommy round dummy]
    )
    assert_equal(filtered1.sort, %w[tummy dummy].sort)

    m2 = MatchObject.build(matcher: 'apple', secret: 'puppy')
    filtered2 = m2.filter_list(
      list: %w[ruppy puppy apple pupty]
    )
    assert_equal(filtered2.sort, %w[puppy ruppy pupty].sort)
    ...
    assert_equal(filtered4.sort, %w[bbbca abbbc zbbac].sort)
  end
end
